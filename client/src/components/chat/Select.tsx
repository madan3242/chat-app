          ...
          <Combobox.Options className="outline outline-[1px] outline-white absolute z-10 mt-2 p-1 max-h-60 w-full overflow-auto rounded-xl bg-purple-500 text-base shadow-lg ring-opacity-5 focus:outline-none sm:text-sm">
            {localOptions.map((option) => (
              <Combobox.Option
                key={option.value}
                value={option}
                className={({ active }) => `${active ?'bg-purple-600' : 'bg-purple-400'} text-white cursor-pointer relative rounded-2xl select-none py-4 pl-3 pr-9 m-1`}
              >
                {({ active, selected }) => (
                  <>
                    <span className={`${selected ? 'font-semibold' : ''} block truncate`}>
                      {option.label}
                    </span>

                    {selected && (
                      <span className={`${active ? 'text-white' : 'text-purple-600'} absolute inset-y-0 right-0 flex items-center pr-4`}>
